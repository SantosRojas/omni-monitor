...
pub fn PatientDashboardPage() -> impl IntoView {
    ...
    view! {
        <div class="page-title">Dashboard del Paciente</div>

        {move || {
            if loading.get() {
                return view! { <LoadingState message="Cargando dashboard..." /> }.into_any();
            }
                        ...
                        {signals.iter().map(|s| {
                            ...
                            view! {
                                <div class="card glass" style="margin-bottom:24px;">
                                    <div class="card-title">{signal.display_name.clone().unwrap_or(signal.internal_name.clone())}
                                    ...
                                    <div class="stats-grid">
                                        <StatsCard
                                            label="Promedio".to_string()
                                            value={signal.average.map(|v| format!("{:.2}", v)).unwrap_or_else(|| "—".to_string())}
                                            color=Some("var(--accent)".to_string())
                                        />
                                        <StatsCard
                                            label="Mínimo".to_string()
                                            value={signal.minimum.map(|v| format!("{:.2}", v)).unwrap_or_else(|| "—".to_string())}
                                            color=Some("var(--success)".to_string())
                                        />
                                        <StatsCard
                                            label="Máximo".to_string()
                                            value={signal.maximum.map(|v| format!("{:.2}", v)).unwrap_or_else(|| "—".to_string())}
                                            color=Some("var(--danger)".to_string())
                                        />
                                        <StatsCard
                                            label="Muestras".to_string()
                                            value={signal.count.to_string()}
                                            color=Some("var(--warning)".to_string())
                                        />
                                    </div>
