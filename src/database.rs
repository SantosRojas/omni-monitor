...
impl DbPool {
    ...
    pub async fn list_therapies_by_patient_paginated(&self, patient_id: i64, page: i64, per_page: i64) -> Result<PaginatedResponse<TherapyWithMachine>, sqlx::Error> {
        ...
        let raw: Vec<TherapyRaw> = match self {
            Self::NoDb => { return Err(sqlx::Error::Configuration("Database not available".into())); },
            Self::Sqlite(p) => {
                sqlx::query_as("SELECT t.id, t.started_at, t.patient_id, t.machine_id, t.status, t.ended_at, m.serial_number, m.software_version, (SELECT mi.ip_address FROM machine_ips mi WHERE mi.machine_id = t.machine_id LIMIT 1) as ip_address, (SELECT mi.port FROM machine_ips mi WHERE mi.machine_id = t.machine_id LIMIT 1) as port, (SELECT s.id FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'g_therapy_mode_set' ORDER BY te.timestamp DESC LIMIT 1) as therapy_type_signal_id, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'g_therapy_mode_set' ORDER BY te.timestamp DESC LIMIT 1) as therapy_type, (SELECT s.id FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'd_kit_type_str' ORDER BY te.timestamp DESC LIMIT 1) as kit_signal_id, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'd_kit_type_str' ORDER BY te.timestamp DESC LIMIT 1) as kit, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_fp_act' ORDER BY te.timestamp DESC LIMIT 1) as filter_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_tmp_act' ORDER BY te.timestamp DESC LIMIT 1) as tmp_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_ep_act' ORDER BY te.timestamp DESC LIMIT 1) as effluent_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_net_rem_flow_act' ORDER BY te.timestamp DESC LIMIT 1) as net_rem_flow, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_pump_fs_mid_flow_act' ORDER BY te.timestamp DESC LIMIT 1) as fs_mid_flow , p.patient_id_str FROM therapies t LEFT JOIN machines m ON t.machine_id = m.id LEFT JOIN patients p ON t.patient_id = p.id WHERE t.patient_id = ? ORDER BY t.started_at DESC LIMIT ? OFFSET ?")
                    .bind(patient_id).bind(per_page).bind(offset).fetch_all(p).await?
            }
            Self::Postgres(p) => {
                sqlx::query_as("SELECT t.id, t.started_at, t.patient_id, t.machine_id, t.status, t.ended_at, m.serial_number, m.software_version, (SELECT mi.ip_address FROM machine_ips mi WHERE mi.machine_id = t.machine_id LIMIT 1) as ip_address, (SELECT mi.port FROM machine_ips mi WHERE mi.machine_id = t.machine_id LIMIT 1) as port, (SELECT s.id FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'g_therapy_mode_set' ORDER BY te.timestamp DESC LIMIT 1) as therapy_type_signal_id, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'g_therapy_mode_set' ORDER BY te.timestamp DESC LIMIT 1) as therapy_type, (SELECT s.id FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'd_kit_type_str' ORDER BY te.timestamp DESC LIMIT 1) as kit_signal_id, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'd_kit_type_str' ORDER BY te.timestamp DESC LIMIT 1) as kit, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_fp_act' ORDER BY te.timestamp DESC LIMIT 1) as filter_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_tmp_act' ORDER BY te.timestamp DESC LIMIT 1) as tmp_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_ep_act' ORDER BY te.timestamp DESC LIMIT 1) as effluent_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_net_rem_flow_act' ORDER BY te.timestamp DESC LIMIT 1) as net_rem_flow, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_pump_fs_mid_flow_act' ORDER BY te.timestamp DESC LIMIT 1) as fs_mid_flow , p.patient_id_str FROM therapies t LEFT JOIN machines m ON t.machine_id = m.id LEFT JOIN patients p ON t.patient_id = p.id WHERE t.patient_id = $1 ORDER BY t.started_at DESC LIMIT $2 OFFSET $3")
    ...
    pub async fn find_therapy_by_id(&self, therapy_id: i64) -> Result<Option<TherapyWithMachine>, sqlx::Error> {
        let raw: Option<TherapyRaw> = match self {
            Self::NoDb => { return Err(sqlx::Error::Configuration("Database not available".into())); },
            Self::Sqlite(p) => {
                sqlx::query_as("SELECT t.id, t.started_at, t.patient_id, t.machine_id, t.status, t.ended_at, m.serial_number, m.software_version, (SELECT mi.ip_address FROM machine_ips mi WHERE mi.machine_id = t.machine_id LIMIT 1) as ip_address, (SELECT mi.port FROM machine_ips mi WHERE mi.machine_id = t.machine_id LIMIT 1) as port, (SELECT s.id FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'g_therapy_mode_set' ORDER BY te.timestamp DESC LIMIT 1) as therapy_type_signal_id, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'g_therapy_mode_set' ORDER BY te.timestamp DESC LIMIT 1) as therapy_type, (SELECT s.id FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'd_kit_type_str' ORDER BY te.timestamp DESC LIMIT 1) as kit_signal_id, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'd_kit_type_str' ORDER BY te.timestamp DESC LIMIT 1) as kit, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_fp_act' ORDER BY te.timestamp DESC LIMIT 1) as filter_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_tmp_act' ORDER BY te.timestamp DESC LIMIT 1) as tmp_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_ep_act' ORDER BY te.timestamp DESC LIMIT 1) as effluent_pressure, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_net_rem_flow_act' ORDER BY te.timestamp DESC LIMIT 1) as net_rem_flow, (SELECT te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_pump_fs_mid_flow_act' ORDER BY te.timestamp DESC LIMIT 1) as fs_mid_flow, p.patient_id_str FROM therapies t LEFT JOIN machines m ON t.machine_id = m.id LEFT JOIN patients p ON t.patient_id = p.id WHERE t.id = ?")
    ...
    pub async fn list_active_therapies(&self) -> Result<Vec<ActiveTherapy>, sqlx::Error> {
        let raw: Vec<ActiveTherapyRaw> = match self {
            ...
            Self::Mssql(db) => db.query_all::<ActiveTherapyRaw>(
                "SELECT t.id as therapy_id, t.patient_id, p.patient_id_str, t.started_at, m.serial_number, (SELECT TOP 1 mi.ip_address FROM machine_ips mi WHERE mi.machine_id = t.machine_id AND mi.is_active = 1) as ip_address, (SELECT TOP 1 mi.port FROM machine_ips mi WHERE mi.machine_id = t.machine_id AND mi.is_active = 1) as port, (SELECT TOP 1 te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_ap_act' ORDER BY te.timestamp DESC) as arterial_pressure, (SELECT TOP 1 te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_vp_act' ORDER BY te.timestamp DESC) as venous_pressure, (SELECT TOP 1 te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_pump_bs_bl_flow_act' ORDER BY te.timestamp DESC) as blood_flow, (SELECT TOP 1 te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_fp_act' ORDER BY te.timestamp DESC) as filter_pressure, (SELECT TOP 1 te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_tmp_act' ORDER BY te.timestamp DESC) as tmp_pressure, (SELECT TOP 1 te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_press_ep_act' ORDER BY te.timestamp DESC) as effluent_pressure, (SELECT TOP 1 te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_net_rem_flow_act' ORDER BY te.timestamp DESC) as net_rem_flow, (SELECT TOP 1 te.physical_value FROM telemetry te JOIN signals s ON te.signal_id = s.id WHERE te.therapy_id = t.id AND s.internal_name = 'c_pump_fs_mid_flow_act' ORDER BY te.timestamp DESC) as fs_mid_flow FROM therapies t JOIN patients p ON t.patient_id = p.id LEFT JOIN machines m ON t.machine_id = m.id WHERE t.status = 'active' ORDER BY t.started_at DESC",
                tp!()
            ).await?,
        };
        ...
        let therapies: Vec<ActiveTherapy> = raw.into_iter().map(|r| {
            ActiveTherapy {
                ...
                filter_pressure: r.filter_pressure,
                tmp_pressure: r.tmp_pressure,
                effluent_pressure: r.effluent_pressure,
                net_rem_flow: r.net_rem_flow,
                fs_mid_flow: r.fs_mid_flow,
...
impl TryFromRow for ActiveTherapyRaw {
    fn try_from_row(row: &Row) -> Result<Self, sqlx::Error> {
        Ok(Self {
            ...
            filter_pressure: col_opt_str(row, "filter_pressure")?,
            tmp_pressure: col_opt_str(row, "tmp_pressure")?,
            effluent_pressure: col_opt_str(row, "effluent_pressure")?,
            net_rem_flow: col_opt_str(row, "net_rem_flow")?,
            fs_mid_flow: col_opt_str(row, "fs_mid_flow")?,
        })
    }
}
