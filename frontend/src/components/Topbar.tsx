import { Palette, Check, LogOut } from 'lucide-react'
...
export function Topbar({ title }: TopbarProps) {
  ...
  return (
    <header className="h-(--header-height) px-3 md:px-8 flex items-center justify-between sticky top-0 z-30 bg-(--topbar-bg) backdrop-blur-md border-b border-(--glass-border)">
      ...
      <div className="flex items-center gap-1 md:gap-3 text-sm text-(--text-secondary) shrink-0">
        <div className="relative" ref={ref}>
          <button
            ...
            className="flex items-center gap-1.5 p-1.5 rounded-sm hover:bg-(--surface-hover) cursor-pointer"
          >
            <Palette className="w-4 h-4" style={{ color: accent }} />
          </button>
          {showAccents && (
            <div className="absolute right-0 top-full mt-1 glass p-3 z-50 min-w-50">
        ...
        <div className="flex items-center gap-1 md:hidden">
          <span className="text-xs truncate max-w-20 text-(--text-secondary)">{user?.full_name || user?.username}</span>
          <button onClick={logout} className="p-1.5 rounded-sm hover:bg-(--surface-hover) text-(--danger) cursor-pointer">
